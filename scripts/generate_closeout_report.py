...
先看终点，四条曲线在 `epoch 99` 的相对位置已经给出本项目最关键的第一层信息。原版首个完整复现终点为 `{formal_val["pearson"]:.4f}`，强基线重跑终点为 `{rerun_val["pearson"]:.4f}`，两者之间存在明显台阶。这一台阶说明训练配方修复带来的影响远大于细小结构改动。直观地看，同一份电路设计图在进入同一模型骨架后，优化过程已经能够把“整体亮暗分布”拟合得更接近真实拥塞图。

再看爬升过程，可以观察到三种节奏。原版复现曲线整体抬升较慢，说明训练流程已经打通，但收敛效率仍有改进空间。强基线曲线在早中期爬升更快，表明优化器设置、学习率与随机种子已经把主干网络的能力调动得更充分。`true Mamba Scheme B` 早期点位低于强基线，中后期逐步追近，最终到达 `{true_val["pearson"]:.4f}`。这一走势与该路线的工程背景是一致的：official-Mamba neck 需要额外的数值稳定设置，起步阶段更谨慎，后程逐步追平。
...
该图的作用是把“训练配方修复”这一大台阶先扣除，再单独观察两条 Mamba 路线带来的净变化。由于比较对象已经是 `{rerun_val["pearson"]:.4f}` 这一强基线，图中的每一个小条形都可以理解为“在已经较强的起点上继续微调后留下的余量”。
...
右图只比较主线基线与 `zero-init BEV Mamba` 的参数量，服务的核心问题是“轻量 neck 到底有多轻”。答案已经较清楚：基线总参数量为 `{summary["params"]["baseline"]["total"]:,}`，新增部分仅有 `{param_overhead["absolute"]:,}`，约 `+{param_overhead["relative_percent"]:.3f}%`。若用一个更直观的比喻来描述，这更接近在原有道路系统中加上一段很短的辅路，用于把信息多传一程，并没有把整座城市重建一遍。
