...
项目主线已经明确: 在完整复现 CircuitFormer 的基础上，于**几乎不增加模型规模**的条件下，在编码器与解码器之间加入轻量级 **BEV Mamba 残差适配层**。最终形成的 `zero-init BEV Mamba` 版本在验证集与测试集上均优于强基线，表明该改进路线具备成立依据。
...
- `Mamba`
  在本项目中的含义并非“完整复现原论文中的 Mamba 主干网络”，而是指一个**受 Mamba / selective state space 思想启发**的二维轻量模块。该模块的实现位置位于 `model/bev_mamba.py`，其主要结构是:
  - 行扫描
  - 列扫描
  - 输入相关的状态更新
  - 门控
...
- 结构位置较克制，仅在 encoder 与 decoder 之间插入 `1` 个 neck block，并未替换原有主干。
...
项目方法的一个显著特点在于，流程没有先将电路强制转换为手工构造图像，也未依赖较重的图预处理步骤；数据表示直接采用“带几何属性的点集合”，并从原始布局元素中学习特征。
...
`model/circuitformer.py` 采用 `segmentation_models_pytorch` 中的 `UnetPlusPlus` 作为解码器，用于对整理后的二维特征图进行细化，并输出单通道拥塞图。

### 3.4 训练目标

`model/model_interface.py` 中的训练损失为**带像素权重的 MSE**:
...
`metrics.py` 对每个样本分别计算 Pearson / Spearman / Kendall，随后对样本级指标取平均。最终分数因此更接近“单张设计图平均预测质量”，不容易受到少数超大样本的主导。
...
该模块位于编码器与解码器之间，输入与输出均为二维 BEV 特征图。依据现有代码，其功能可表述为: 在不改变主干框架的前提下，为 BEV 特征图加入一条轻量级的行列扫描残差通道，用于补充较大范围的上下文传播。
...
对于拥塞图而言，这种机制相当于允许某个网格位置同时参考左侧、右侧、上方、下方的较远区域，而不局限于局部卷积核覆盖的邻域。
...
因此，当前位置的判断依据不再局限于邻近几个像素，而会吸收更大范围的版图组织方式。
...
与平均池化或固定卷积相比，该更新方式允许不同位置、不同通道采用不同的历史保留比例与当前写入比例。这一判断可以直接由 `delta_proj` 生成逐位置、逐通道 $\delta_t$ 的代码实现支持。
...
这一点与原论文中的“selectively propagate or forget information”在思想上相一致。
...
- 在规则、平滑的区域，闸门可能较小，避免远处信息过度干扰局部稳定结构
...
该模块并未替换原有编码器与解码器，而是作为中间适配层接入现有主干。原版 CircuitFormer 负责:
...
1. 让状态空间模型参数成为输入的函数，从而根据当前 token **选择性地传播或遗忘信息**
2. 在此基础上，设计适合高效实现的并行扫描算法

与原论文对照后，可以将本项目中的 `BEV Mamba neck` 更谨慎地定位为**受 Mamba / selective state space 思想启发的二维轻量化适配模块**。依据来自两部分:
...
- **本项目中的简化与改写**
  - 原论文中的 selective SSM 使用更完整的输入相关参数化；从当前代码看，本项目实现只显式出现 `delta_proj` 与 `value_proj` 两个线性层，未见原论文中完整 selective SSM 参数化在代码中的逐项展开
  - 原论文中强调高效实现的并行扫描；从当前代码看，`SequenceStateSpace.forward` 在时间维上采用显式 `for` 循环，未实现原论文中的硬件感知并行扫描内核
  - 原论文将 Mamba 作为整体 backbone；当前项目仅在编码器与解码器之间插入一个 `neck`，属于局部适配层
  - 原论文面向一般一维长序列；当前项目将相关思想迁移到二维 BEV 表征，通过行扫描与列扫描处理版图空间依赖
  - 原论文标题中的 “Linear-Time” 对应模型形式与高效实现框架；当前项目代码没有单独给出复杂度证明或实现层面的效率 benchmark，因此不宜将原论文中的效率结论直接转述为本项目结论

从上述对照可以得到一个更准确的表述:

- 当前实现未逐项复刻原论文全部细节
- 当前实现保留了“输入相关选择性状态更新”这一核心思想的简化形式
- 当前实现完成了面向二维拥塞图任务的结构改写

据此，更严格的表述应为: 当前模块可称为 `BEV Mamba neck`，但其含义应限定为“**受 Mamba 原理启发、面向二维 BEV 场景设计的轻量残差扫描模块**”。这一表述与当前代码实现更一致。
...
## 5. `zero-init` 的训练稳定性意义

`model/bev_mamba.py` 中提供了 `out_proj_init_zero=True` 这一设置，对应如下代码逻辑:

- 新增分支最后一层 `out_proj` 的权重被初始化为 0

在参数初始化时，由于 `out_proj` 权重被显式置零且该层不含 bias，Mamba 分支经 `out_proj` 后输出精确为 0，整个块的输出满足:

$$
\mathrm{output} = \mathrm{residual}
$$

这意味着模块在初始化时就是恒等映射，主干网络已有表征不会被新增分支立即改写。
...
- 在一个已经运转稳定的生产线上，新装上的辅助机械臂如果一开始就大幅改写主流程，整体系统很容易失稳
- 若机械臂初始几乎不动作，只先观察、再逐步增加动作幅度，则整体系统更容易保持稳定

`zero-init` 在当前结构中发挥的正是这一类作用: 初始阶段尽量保持原主干行为，随后再逐渐学习有效修正。
...
首次 BEV Mamba 尝试为 `exp/congestion_bev_mamba_run1_2026-04-13_09-58-40_UTC`。日志中明确记录了 `FileNotFoundError`。问题来源于数据路径指向 `../datasets/...` 下一个不存在的 `.npy` 文件。
...
需要重点关注的是，三个测试指标均出现同步提升，其中 Spearman 与 Kendall 的增幅高于 Pearson。基于现有指标，可以给出如下推断: 新模块带来的改进不仅体现在绝对值接近程度上，也体现在拥塞图高低关系的排序一致性上。该解释属于依据现有数据进行的推断，与现有结果相符。
...
因此，此处“轻量”并不是笼统形容，而是指: 仅用一个两万九千级别参数的小型中间模块，对一个两千三百六十万级别的主模型做增量增强。

这一结果表明，性能提升并非依赖大幅扩容模型规模，结构设计本身发挥了更主要的作用。
...
- 在强基线已处于较高水平的条件下，`zero-init BEV Mamba` 仍能继续提升最终点位。
...
该图突出结构改造的净贡献。重点在于，在较强 baseline 上仍能观察到稳定正增益。
...
3. **用 zero-init 控制新模块的早期扰动。**
   该设计提升了训练初期的稳定性，使新增分支以残差修正项的形式逐步介入。
...
为保证陈述可核验，保留如下事实说明: `zero-init` 首轮训练的早期标准输出未完整保留为标准 `train.log`，因此 `epoch 89` 之前的中间验证点缺少与另外两条实验相同粒度的完整可视化。报告中关于该实验的最终结论仅使用**已保存 checkpoint、恢复日志与重新跑出的测试日志**，未使用猜测性补点。
...
- 中断恢复链路已经验证
- 最终验收模型在验证集与测试集上均优于强基线
- 提升未依赖大幅加参数

建议将 `zero-init BEV Mamba` 版本作为正式收口版本，并据此完成最终答辩与汇报材料。
